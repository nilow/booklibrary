...
class Specimens extends Component{
  ...
  listPortion(){
    const token = localStorage.getItem('token');
  	axios.get('http://backng.nilow13.usermd.net/api/bookspecimens2/' + this.props.match.params.id + '/' + this.state.offset +'?token=' + token)
      .then(res => {
  ...
  listSearch(){
    const token = localStorage.getItem('token');
    axios.get('http://backng.nilow13.usermd.net/api/searchbookspecimens2/'+ this.props.match.params.id + '/' + this.state.searchtext + '/' + this.state.offset + '?token=' + token)
      .then(res => {
  ...
  render(){
    return(
      <div className="container">
        <div className="top">
          <div><a className="btn-link" onClick={this.specimenAdd}><PlusSvg/></a><h3 className="top__title">Egzemplarze</h3></div>
