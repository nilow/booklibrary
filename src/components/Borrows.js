...
class Borrows extends Component{
  ...
  listPortion(){
    const token = localStorage.getItem('token');
  	axios.get('http://backng.nilow13.usermd.net/api/borrows2/'+ this.state.offset +'?token=' + token)
      .then(res => {
  ...
  listSearch(){
    const token = localStorage.getItem('token');
    axios.get('http://backng.nilow13.usermd.net/api/searchborrows2/'+ this.state.searchtext + '/' + this.state.offset + '?token=' + token)
      .then(res => {
  ...
  render(){
    return(
      <div className="container">
        <div className="top">
          <div><Link to={"/currentborrows"} className="btn-link"><EyeSvg/></Link><a className="btn-link" onClick={this.borrowAdd}><PlusSvg/></a><h3 className="top__title">Wypożyczenia</h3></div>
