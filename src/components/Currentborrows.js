...
class Currentborrows extends Component{
  ...
  listPortion(){
    const token = localStorage.getItem('token');
  	axios.get('http://backng.nilow13.usermd.net/api/currentborrows2/'+ this.state.offset +'?token=' + token)
      .then(res => {
  ...
  listSearch(){
    const token = localStorage.getItem('token');
    axios.get('http://backng.nilow13.usermd.net/api/searchcurrentborrows2/'+ this.state.searchtext + '/' + this.state.offset + '?token=' + token)
      .then(res => {
  ...
  render(){
    return(
      <div className="container">
        <div className="top">
          <div><a className="btn-link" onClick={this.borrowAdd}><PlusSvg/></a><h3 className="top__title">Bieżące wypożyczenia</h3></div>
