...
class Availablespecimens extends Component{
  ...
  listPortion(){
    const token = localStorage.getItem('token');
  	axios.get('http://backng.nilow13.usermd.net/api/availablespecimens2/'+ this.state.offset +'?token=' + token)
      .then(res => {
  ...
  listSearch(){
    const token = localStorage.getItem('token');
    axios.get('http://backng.nilow13.usermd.net/api/searchavailablespecimens2/'+ this.state.searchtext + '/' + this.state.offset + '?token=' + token)
      .then(res => {
  ...
  render(){
    return(
      <div className="container">
        <div className="top">
          <div><a className="btn-link" onClick={this.specimenAdd}><PlusSvg/></a><h3 className="top__title">Dostępne egzemplarze</h3></div>
...
export default Availablespecimens
